...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            int requestlen = strlen(line);
            // counter to count spaces
            int count = 0;
            
            // iterate through string & count spaces
            ...
            // if does not meet criteria, return error
            if (count != 2)
            {
                error(400);
                continue;
            ...
            char* sp = strchr(line,' ');
            // create method length
            int methodlen = sp - line;
            // create a buffer to store our method
            char method[methodlen + 1];
            // place dat bad boy in there
            strncpy(method,line,methodlen);
            ...
            // sp points to space, next character must be /, check 
            if (sp[1] != '/')
            {
                dprintf(cfd, "error is here\r\n");
                error(501);
                continue;
            }
            
            // create a char* that points to end of request-target
            char* sp2 = strchr((sp+1),' ');
            
            // get the length of request-target
            int targetlen = sp2 - (sp+1);
            
            // create a buffer for request-target
            char reqtarget[targetlen+1];
            
            // copy length of request target into buffer
            strncpy(reqtarget,(sp+1),targetlen);
            
            // add null character
            reqtarget[targetlen] = '\0';
            
            // check to see if request-target does not have quotes
            ...
            // lets check HTTP-version
            if (strcmp(sp2+1, "HTTP/1.1\r\n") != 0)
            {
                error(505);
                continue;
            }
            ...
            // create pointer to query if it exists
            char* questions = strchr(reqtarget,'?');
            
            // create query string
            char* query = NULL;
            
            // create abspathlen and initialize it
            int abspathlen = 0;
            
            // if question mark was found
            if (questions != '\0')
            {
                // query is equal to our query till '\0' that we added
                query = questions+1;
                
                // lenth of our absolute path is request target - the end of the path (start of query)
                abspathlen = questions - reqtarget;
                
            }
            // if question mark was not found, query stays NULL
            else
            {
                // and absolute path length is the length of our request-target.
                abspathlen = strlen(reqtarget);
            }
            
            // create a buffer for our absolute path
            char absolutebuffer[abspathlen+1];
            
            // store the absolute path in the buffer
            strncpy(absolutebuffer,reqtarget,abspathlen);
            
            // add null to end of string
            absolutebuffer[abspathlen] = '\0';
            
            // create a counter for .
            int dot = 0;
            
            for (int i = 0; i < strlen(absolutebuffer); i++)
            {
                if (absolutebuffer[i] == '.')
                {
                    dot++;
            ...
            // create root length variable
            int rootlen = strlen(root);
            
            // put into different variable
            int pathlen = abspathlen + rootlen;
            // create path buffer
            char path[pathlen + 1];
            
            // put root into path buffer
            strcpy(path,root);
            
            // concatinate root and abspath
            strcat(path,absolutebuffer);
            
            // add null to it
            path[pathlen] = '\0';

            
            // check if path exists, if access exists, then returns 0
            int existResult = access(path,F_OK);
            ...
            // check if path is readable, if readable, return 0
            ...
            /*
            // lets get path's extension
            char* period = strchr(path,'.');
            
            // get to start of extension
            char* ext = period + 1;
